...
export default {
    ...
    setup() {
        ...
        const numberOfUsers = ref([5000, 15000, 8000, 12000, 14000, 16000, 18000])
        ...
        const getSalesOrderData = async () => {
            ...
            try {
                const response = await axios.post('https://tayarpro-service-1082564059873.us-central1.run.app/get_order_statistics', {}, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                months.value = response.data.months;
                numberOfUsers.value = response.data.orders; // Update the value of the ref
