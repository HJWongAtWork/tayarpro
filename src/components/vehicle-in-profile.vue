      ...
      <v-card-text>
        <v-form ref="form" v-model="isValidAddVehicle">
          <v-text-field
            ...
            required
            :disabled="
              vehicleDialogAction === 'delete' || vehicleDialogAction === 'edit'
            "
          ></v-text-field>
          <v-select
            v-model="brandInput"
            :items="brandSelections"
            label="Select Brand"
            outlined
            dense
            :rules="[rules.required]"
          ...
          <v-select
            v-model="modelInput"
            :items="modelSelections"
            label="Select Model"
            outlined
            dense
            :rules="[rules.required]"
            required
            :disabled="vehicleDialogAction === 'delete' || brandInput === ''"
          ></v-select>
          <v-select
            v-model="yearInput"
  ...
  methods: {
    ...
    async getBrandSelections() {
      try {
        ...
        this.brandSelections = this.brandSelections.map((brand) =>
          brand.toUpperCase()
        );
      } catch (error) {
    ...
    async getModelSelections() {
      try {
        ...
        this.modelSelections = this.modelSelections.map((brand) =>
          brand.toUpperCase()
        );
      } catch (error) {
        if (axios.isAxiosError(error)) {
