      ...
      <v-form v-model="addFormValid" :disabled="userType === ''">
        <v-row>
          ...
          <v-row v-if="selectedCarStr === 'Add New Car'">
            <v-col cols="12" sm="12" md="4">
              <v-text-field v-model="newBrand" label="Enter Car Brand" outlined dense :rules="[rules.required]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4">
              <v-text-field v-model="newModel" label="Enter Car Model" outlined dense :rules="[rules.required]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4">
              <v-text-field type="number" v-model="newYear" label="Enter Car Year" outlined dense :rules="[rules.required, rules.year, rules.numberOnly]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4">
              <v-text-field v-model="newNumberPlate" label="Enter Number Plate" outlined dense :rules="[rules.required]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4">
              <v-text-field v-model="newTyreSize" label="Enter Tyre Size(example: 215/65R15)" outlined dense :rules="[rules.tyreFormat]"></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="4" style="font-size: 13px;">
              *Tyre Size is Optional. You may proceed to confirm the appointment.
            </v-col>
          </v-row>
        </v-row>
        <v-row>
  ...
  <script>
    ...
    export default {
      ...
      setup() {
        ...
        const { vehicles, plateNumberInput, brandInput, modelInput, yearInput, tyreSizeInput, fetchVehicles, getLatestVehicleId, addVehicle } = vehicleComposable();
        ...
        const addAppointment = () => {
          ...
          if (newBayInt && newStartHour) {
            ...
            if (!newBrand.value && !newModel.value && !newYear.value && selectedCarObj.value) {
      ...
      mounted() {
        this.formattedCarText.push("Add New Car");
      },
      computed: {
        formattedCarText() {
          return this.vehicles.map((vehicle) => `${vehicle.brand} ${vehicle.model} ${vehicle.year} (${vehicle.plateNumber})`);
        },
      },
      methods: {
        updateDisabledHours() {
