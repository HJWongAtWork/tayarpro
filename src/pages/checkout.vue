<template>
  <v-container>
    <v-stepper v-model="step">
      ...
      <v-stepper-window>
        <v-stepper-window-item value="1">
          <v-card title="Select Payment Method" flat>
            <v-form>
              <v-radio-group v-model="paymentType" :rules="[rules.required]" @input="handleInput">
                <v-radio label="Credit Card" value="CreditCard" color="red"></v-radio>
                <v-radio label="Bank Transfer" value="BankTransfer" color="red"></v-radio>
                <v-radio label="E Wallet" value="EWallet" color="red"></v-radio>
              </v-radio-group>
              <template v-if="paymentType === 'CreditCard'">
                <v-text-field v-model="cardNumber" label="Card Number" :rules="[rules.required]"></v-text-field>
                <v-text-field v-model="cardExpiry" label="Expiry Date" :rules="[rules.required]"></v-text-field>
                <v-text-field v-model="cardCVC" label="CVC" :rules="[rules.required]"></v-text-field>
              </template>
              <template v-else-if="paymentType === 'BankTransfer'">
                <v-text-field v-model="bankAccountNumber" label="Bank Account Number" :rules="[rules.required]"></v-text-field>
                <v-text-field v-model="bankName" label="Bank Name" :rules="[rules.required]"></v-text-field>
              </template>
              <template v-else-if="paymentType === 'EWallet'">
                <v-text-field v-model="eWalletType" label="E-Wallet Type" :rules="[rules.required]"></v-text-field>
                <v-text-field v-model="eWalletID" label="E-Wallet Username/Phone Number" :rules="[rules.required]"></v-text-field>
              </template>
            </v-form>
          </v-card>
        ...
        <v-stepper-window-item value="3">
          <v-card title="Details Confirmation" flat>
            ...
            <v-table density="comfortable">
              <tbody>
                <tr>
                  <td class="table-cell-small"><h3>Payment Details</h3></td>
                ...
                <tr>
                  ...
                  <td class="table-cell-small">RM {{ subtotalDisplay }}</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td class="table-cell-small">RM {{ totalDisplay }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small"><h3>Appointment Details</h3></td>
                  <td class="table-cell-small"></td>
                </tr>
                <tr>
                  <td class="table-cell-small">Appointment ID</td>
                  <td class="table-cell-small">{{ newAppointment.id }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Date</td>
                  <td class="table-cell-small">{{ formatDateToReadable(newAppointment.dateTime) }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Time</td>
                  <td class="table-cell-small">{{ newAppointment.dateTime.toLocaleTimeString() }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Service</td>
                  <td class="table-cell-small">{{ newAppointment.service }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Bay Number</td>
                  <td class="table-cell-small">{{ newAppointment.bay }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Status</td>
                  <td class="table-cell-small">{{ newAppointment.status }}</td>
                </tr>
                <tr>
                  <td class="table-cell-small">Car Details</td>
                  <td class="table-cell-small">
                    {{ newAppointment.brand }} {{ newAppointment.model }} {{ newAppointment.year }}
                  </td>
                </tr>
              </tbody>
            </v-table>
      ...
      <v-row>
        <v-col cols="6">
          <v-btn v-if="step > 0" variant="text" @click="step--" style="margin: 10px;">
            <v-icon left>mdi-arrow-left</v-icon>
            Back
          </v-btn>
        </v-col>
        <v-col cols="6" class="text-end">
          <v-btn color="#FF6875" @click="proceedToNextStep" :loading="isLoading" :disabled="validateCurrentStep()" style="margin: 10px;">
