...
export default defineComponent({
  ...
  setup() {
      ...
      try {
        ...
        if (itemId) {
          const response = await axios.get(
            `http://tayar.pro/get_product_by_id?tyre_id=${itemId}`
          );
    ...
    const addToCart = async () => {
      try {
        const cartResponse = await axios.get(
          `http://localhost:8000/cart/${userStore.currentUser.accountid}/${tyre.value.itemid}`
        );
      ...
      try {
        if (tyre.value && tyre.value.brandid) {
          const response = await axios.get<RecommendedTyre[]>(
            "http://localhost:8000/get_random_tyres"
          );
    ...
    const goToTyreDetails = (tyre: RecommendedTyre) => {
      localStorage.setItem("currentTyreItemId", tyre.itemid);
      router
        .push({
          name: "/[description]",
          params: { description: encodeURIComponent(tyre.description) },
        })
        .then(() => {
