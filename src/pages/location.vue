...
export default {
  data() {
    return {
      locations: [
        ...
        {
          title: "TayarPro Sunway Business Park (Opening Soon)",
          address: {
            unitNO: "38",
  ...
  mounted() {
    ...
    const map = L.map("map").setView(
      [this.locations[0].latt, this.locations[0].lng],
      13
    );
    ...
    let marker1 = L.marker([
      this.locations[0].latt,
      this.locations[0].lng,
    ]).addTo(map);
    let marker2 = L.marker([
      this.locations[1].latt,
      this.locations[1].lng,
    ]).addTo(map);

    marker1
      .bindPopup("<b>TayarPro Tiara Villa</b>", {
        closeOnClick: false,
        autoClose: false,
      })
      .openPopup();
    marker2
      .bindPopup("<b>TayarPro Sunway Business Park</b>", {
        closeOnClick: false,
        autoClose: false,
      })
      .openPopup();
